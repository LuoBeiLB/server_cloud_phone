# =============================================================================
# patch-follow-windows.ps1 —— 给本机已部署的 ws-scrcpy dist 注入 follow 事件补丁
# 不动样式、不重跑构建、不重启服务：注入后浏览器 Ctrl+F5 强刷即可生效。
#
# 用法（本地 Windows）：
#   先 cd 到本地 ws-scrcpy 根目录（里面能看到 dist 文件夹），然后执行：
#   powershell -ExecutionPolicy Bypass -File patch-follow-windows.ps1
#
...
# 幂等：已有补丁则跳过，重复执行安全。
# =============================================================================
$p = Join-Path (Get-Location) "dist\index.html"
if (-not (Test-Path $p)) {
    Write-Host "!! 找不到 $p —— 请先 cd 到 ws-scrcpy 根目录（dist 的上一级）再跑"
    exit 1
}
$html = [IO.File]::ReadAllText($p)
if ($html.Contains("cp-scrcpy-follow")) {
    Write-Host "已有 follow 补丁，跳过"
    exit 0
}
$js = @'
<script data-cp-follow>
...
Write-Host "==> 已注入 follow 事件补丁: $p"
Write-Host "    浏览器端 Ctrl+F5 强刷生效"
